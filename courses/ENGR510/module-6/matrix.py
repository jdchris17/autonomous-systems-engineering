...
class Matrix:
    ...
    def inverse(self) -> "Matrix":
        ...
        if not self.is_square:
            raise ValueError(f"cannot invert a non-square {self.shape} matrix")

        n = self.num_rows
        augmented = [row[:] + identity_row for row, identity_row in zip(self.rows, Matrix.identity(n).rows)]
        ...
        for col in range(n):
            ...
            augmented[col] = [value / pivot_val for value in augmented[col]]

            for r in range(n):
                if r != col:
                    factor = augmented[r][col]
                    augmented[r] = [
                        rv - factor * cv for rv, cv in zip(augmented[r], augmented[col])
                    ]
