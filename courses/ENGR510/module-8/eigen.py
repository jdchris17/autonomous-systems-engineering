...
def _complete_orthonormal_basis(existing_vectors: list, n: int) -> list:
    """Find a unit vector orthogonal to every vector in `existing_vectors`
    (assumed already orthonormal), by Gram-Schmidt-ing a standard basis
    vector against them. Used to fill in the "don't care" directions for
    a matrix's zero eigenvalues.
    """
    from least_squares import gram_schmidt

    for i in range(n):
        candidate = [1.0 if j == i else 0.0 for j in range(n)]
        try:
            extended = gram_schmidt(existing_vectors + [candidate])
            return extended[-1]
        except ValueError:
            continue
    raise ValueError("could not complete an orthonormal basis")
...
def diagonalize_symmetric(a: Matrix, num_iterations: int = 5000, tol: float = 1e-10, seed=None) -> tuple[Matrix, Matrix]:
    ...
    for _ in range(n):
        if all(abs(value) < 1e-10 for row in working.rows for value in row):
            # A was exactly rank-deficient: deflation has removed every
            # nonzero eigenvalue, and power iteration has nothing left to
            # converge to. The remaining eigenvalues are (numerically) zero
            # -- fill in an arbitrary direction, orthogonal to what we've
            # already found, to complete the basis.
            eigenvalues.append(0.0)
            eigenvectors.append(_complete_orthonormal_basis(eigenvectors, n))
            continue
